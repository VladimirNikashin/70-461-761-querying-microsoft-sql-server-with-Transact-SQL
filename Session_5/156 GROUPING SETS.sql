...
GO
-- GROUP BY CUBE is the sme as GROUP BY ROLLUP only difference is that cube gives 7 totals
...
ON E.EmployeeNumber = A.EmployeeNumber
GROUP BY GROUPING SETS ((E.Department, E.EmployeeNumber, A.AttendanceMonth), (E.Department))
ORDER BY COALESCE (Department, 'zzzzzzzzzz'), COALESCE (EmployeeNumber, 99999), COALESCE (AttendanceMonth,'2100-01-01')
GO
...
FROM tblEmployee AS E JOIN tblAttendance AS A
ON E.EmployeeNumber = A.EmployeeNumber
GROUP BY ROLLUP ((E.Department, E.EmployeeNumber, A.AttendanceMonth), (E.Department))
ORDER BY COALESCE (Department, 'zzzzzzzzzz'), COALESCE (EmployeeNumber, 99999), COALESCE (AttendanceMonth,'2100-01-01')
GO
